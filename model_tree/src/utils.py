...
def load_csv_data(input_csv_filename, mode="clf", verbose=False):
    ...
    df['l2_ratio'] = 0
    df['l1_ratio'][df['penalty'] == 'none'] = 0
    df['l2_ratio'][df['penalty'] == 'none'] = 0
    df['l1_ratio'][df['penalty'] == 'l1'] = 1
    df['l2_ratio'][df['penalty'] == 'l1'] = 0
    df['l1_ratio'][df['penalty'] == 'l2'] = 0
    df['l2_ratio'][df['penalty'] == 'l2'] = 1
    df['l2_ratio'][df['penalty'] == 'elasticnet'] = 1 - df['l1_ratio'][df['penalty'] == 'elasticnet']
    df['n_jobs'][df['n_jobs'] == -1] = 16
    del df['penalty']

    df_header = df.columns.values  # header
    header = list(df_header)
    N, d = len(df), len(df_header) - 1
    X = np.array(df.drop(['time'], axis=1))  # extract X by dropping y column
    y = np.array(df['time'])  # extract y
